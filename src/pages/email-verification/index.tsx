  ...
  useEffect(() => {
    ...
    timeoutId = setTimeout(() => {
      if ('token' in router.query && !open) {
        ...
        userService
          .emailVerification(router.query.token as string)
          .then((rep: any) => {
            console.log(' rep ', rep)
            setLoading(false)
            setIsError(false)
            ...
            <Box sx={{ my: 6 }}>
              <>
                <Typography variant='h3' sx={{ mb: 1.5 }}>
                  {t('La validation de votre email est en cours...')} 👁️‍🗨️
                </Typography>
              ...
              <Slide in={open} direction='left' {...(open ? { timeout: 500 } : {})}>
                ...
                >
                  {t(
                    "Votre email a été confirmé avec succès. 👂 Veuillez bien rester à l'écoute, notre équipe vous contactera dans les plus brefs délais."
                  )}
                </Alert>
              </Slide>
            </Box>
